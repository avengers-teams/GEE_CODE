...
var LC57_BANDS = ['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7','ST_B6','QA_PIXEL'];
var LC89_BANDS = ['SR_B2','SR_B3','SR_B4','SR_B5','SR_B6','SR_B7','ST_B10','QA_PIXEL'];
var STD_NAMES = ['Blue', 'Green', 'Red', 'NIR', 'SWIR1', 'SWIR2','TIR','QA_PIXEL'];

// 加载 Landsat 5/7/8/9 数据集
if (year >= 2013){// landsat8/9
...
function calculateIndices(image) {
    ...
    var optical = optical.addBands(thermal).addBands(image.select('QA_PIXEL'));
    
    if (year >= 2013) {
        optical = optical.select(LC89_BANDS, STD_NAMES);
    } else {
        optical = optical.select(LC57_BANDS, STD_NAMES);
    }

    var wetness;
    
    if (year >= 2013) {
        // Landsat 8/9 OLI 湿度指数系数
        wetness = optical.expression(
...
function calculatePCA(image) {
  ...
  var wetCoef = eigenVectors.get([1, 0]);
  var lstCoef = eigenVectors.get([2, 0]);
  var ndbsiCoef = eigenVectors.get([3, 0]);
  
  // 创建调整矩阵
  var adjustMatrix = ee.Array([
    [ee.Number(ndviCoef).gt(0).multiply(2).subtract(1), 0, 0, 0],
    [0, ee.Number(wetCoef).gt(0).multiply(2).subtract(1), 0, 0],
    [0, 0, ee.Number(lstCoef).lt(0).multiply(2).subtract(1), 0],
    [0, 0, 0, ee.Number(ndbsiCoef).lt(0).multiply(2).subtract(1)]
  ]);
  
  
  // 仅调整PC1的方向（其他主成分保持不变）
  var adjustedPC1 = eigenVectors.slice(1,0,1).matrixTranspose().matrixMultiply(adjustMatrix).matrixTranspose();  // 第一列乘以符号
  // print(adjustedPC1)
  var otherPCs = eigenVectors.slice(1,1,4);  // 其他列不变
  // print('PC1 dimensions:', adjustedPC1.length()); // 应返回[4,1]
  // print('Other PCs dimensions:', otherPCs.length()); // 应返回[4,3]
  var adjustedEigenVectors = ee.Array.cat([adjustedPC1, otherPCs], 1);  // 重新组合
  print('adjustedEigenVectors',adjustedEigenVectors)
  ...
  var eigenVectorsTransposed = adjustedEigenVectors.transpose();
...
function getMinMax(image){
 var Maxmin = image.clip(region).reduceRegion({
  ...
  var Mean = image.clip(region).reduceRegion({
  reducer:ee.Reducer.mean(),
  geometry:geometry,
  maxPixels:1e10,
  scale:30
...
print('RSEI_minMaxMean:',getMinMax(rsei),getMean(rsei))
print('Greenness_minMaxMean:',getMinMax(greenness),getMean(greenness))
print('Wetness_minMaxMean:',getMinMax(wetness),getMean(wetness))
print('Heat_minMaxMean:',getMinMax(heat),getMean(heat))
print('Dryness_minMaxMean:',getMinMax(dryness),getMean(dryness))
