...
var year = '2019'; 
function adjustDirection(eigenVectorsList, position, direction) {
  var row = position[0];
  var col = position[1];
  var currentValue = eigenVectorsList[row][col];
  
  if (direction === -1) {
    eigenVectorsList[row][col] = -Math.abs(currentValue);
  } else if (direction === 1) {
    eigenVectorsList[row][col] = Math.abs(currentValue);
  }
  return ee.Array(eigenVectorsList);
}
...
function calculatePCA(image) {
 ...
 print('PCA Results:', features);
  ...
  print(eigenVectors)
  // 调整特征向量
  var eigenVectorsList = eigenVectors.getInfo();
  //调整方向
  print(eigenVectorsList)
  eigenVectors  = adjustDirection(eigenVectorsList,[0, 0],1)
  eigenVectors  = adjustDirection(eigenVectorsList,[1, 0],1)
  eigenVectors  = adjustDirection(eigenVectorsList,[2, 0],1)
  eigenVectors  = adjustDirection(eigenVectorsList,[3, 0],1)
  print(eigenVectors)

  var adjustedEigenVectors = eigenVectors.matrixMultiply(eigenVectors);
  print(adjustedEigenVectors)
  // 将调整后的特征向量转换为图像格式
...
print('RSEI_mean:',getMean(rsei))
print('Greenness_mean:',getMean(greenness))
print('Wetness_mean:',getMean(wetness))
print('Heat_mean:',getMean(heat))
print('Dryness_mean:',getMean(dryness))
